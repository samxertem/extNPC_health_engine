...
<p align="center">
  <a href="https://github.com/samxertem/samara_engine/actions/workflows/ci.yml"><img alt="CI" src="https://github.com/samxertem/samara_engine/actions/workflows/ci.yml/badge.svg"></a>
  <img alt="version"  src="https://img.shields.io/badge/version-0.5.0-1a1a19?style=flat-square">
  <img alt="licence"  src="https://img.shields.io/badge/licence-Apache--2.0-1a1a19?style=flat-square">
...
**What is drawn, and where the line is.** Each villager is nine submeshes over
four shared surfaces: skin, flesh, hair and cloth. Skin carries a detail map
reduced to pure luminance, so lips, nail beds and the shading under a brow are
the texture's and the *tone* remains the engine's measured `skin_tone` on the
Del Bino locus. Eyes are drawn from MakeHuman's own textures because an
eyeball is not one colour, and eyebrows and eyelashes from alpha cutouts
because a brow is not one shape: between 1.5 and 21.2 percent of one of those
cards is hair and the rest is the gaps between strands. Brows take the
villager's measured `hair_pigment`, and the viewer withholds that colour
unless the cutout that gives it the right shape is installed, because a dark
card without its alpha is a solid rectangle. The screenshots above predate
this pass and show the flat-shaded version.

Cosmetic channels are labelled as cosmetic wherever they appear. Clothing
colour and hairstyle are a hash of the villager's name and carry no biology;
`bodies.json` records, per channel, which is measured and which is convention.
...
<p align="center">
  <img src="docs/showcase/life-stages.gif" width="100%"
       alt="Darius animated across seven life stages, from a 49 cm infant in year 16 to a 164 cm man of 84 in year 100, with the viewer's inspector panel showing his height and life stage at each step">
</p>

<sub>The same seven captures as a loop, so the change is visible rather than
inferred. Every frame is a real capture of the viewer at that year: nothing is
tweened, because a body between two of these is a body the engine never
produced. Watch the inspector on the right, which reads
<i>164.2 cm &rarr; 171 adult</i> in the last frame: the engine reporting his
senescent loss against his own unchanged genetic endpoint. Rebuilt by
<code>python docs/make_animations.py</code>.</sub>
...
unity/                UPM package: reads a bundle, renders the village
  ...
  Tests/                143 EditMode tests, incl. a generated parity fixture

mpfb/                 the genome -> parametric mesh -> rigged body bake path
tests/                the Python suite, 1,272 tests
...
docs/showcase/        the screenshots above, from docs/make_showcase.py
                      and the animation, from docs/make_animations.py
```
...
- **The validation harness has no automated gate.** The test suite and the
  figure manifests now run in CI, but `full_report()` returns a formatted
  string and nothing asserts on it: its twenty verdicts are gated by a person
  reading the output. A law can start failing without any automation noticing,
  and for a project whose central claim is that its results are reproducible,
...
`python docs/make_diagrams.py`, `python docs/make_showcase.py` and
`python docs/make_animations.py`. The first needs nothing but the standard
library; the other two read the full-resolution captures in
`article_screenings/` and `outputs/paper/`, which are not tracked, and say so
and skip rather than fail when those are absent.
